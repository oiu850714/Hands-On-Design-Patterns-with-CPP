...
public:
  ...
  friend Multipliable operator*(Multipliable lhs, basic_type rhs) {
    return Multipliable(lhs.val_ * rhs);
  }
  friend Multipliable operator*(basic_type lhs, Multipliable rhs) {
    return Multipliable(lhs * rhs.val_);
  }
  friend Multipliable operator/(Multipliable lhs, Multipliable rhs) {
    return Multipliable(lhs.val_ / rhs.val_);
  }
...
int main() {
  ...
  }
  {
    ...
    // cout << "(i + j) * (k + 3): " << ((i + j) * (k + 3)) << endl; // Order
    // problem, i + j is Addable not Multipliable!
  }
  {
    ...
    cout << "i * 3: " << (i * 3) << ", 7 / k: " << (7 / k) << endl;
    // cout << "(i * j) + (k / 3): " << ((i * j) + (k / 3)) << endl; // Order
    // problem, i + j is Multipliable not Addable!
    cout << "(i + j) * (k + 3): " << ((i + j) * (k + 3)) << endl;
  }
}

...
public:
  ...
  auto operator()(Func func, Args &&...args) {
    ...
    cout << "Invoking " << typeid(func).name() << " "
         << abi::__cxa_demangle(typeid(func).name(), buf, &len, &status)
         << endl;
    // auto res = (this->*func)(args ...); // Only if func() does not return
    // void!
  ...
  // double attack() { double res = U::attack(); cout << "Attack: " << res <<
  // endl; return res; }
  double defense() {

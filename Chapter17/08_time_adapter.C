...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, const USA_Date &d) {
    ...
    out << local_tm.tm_mon + 1 << "/" << local_tm.tm_mday << "/"
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, const European_Date &d) {
    tm local_tm = *localtime(&d.t_);
    out << local_tm.tm_mday << "/" << local_tm.tm_mon + 1 << "/"
        << local_tm.tm_year + 1900;
    return out;
  }
...
public:
  ...
  friend std::ostream &operator<<(std::ostream &out, const ISO_Date &d) {
    tm local_tm = *localtime(&d.t_);
    out << local_tm.tm_year + 1900 << "/" << local_tm.tm_mon + 1 << "/"
        << local_tm.tm_mday;
    return out;
  }
...
int main() {
  ...
  cout << "Today in USA: " << USA_Date(now)
       << ", Europe: " << European_Date(now) << ", ISO: " << ISO_Date(now)
       << endl;
}

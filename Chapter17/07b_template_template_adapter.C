...
template <typename T> struct DeleteSmallHeap : public DeleteHeap<T, SmallHeap> {
...
class SmartPtr {
public:
  explicit SmartPtr(T *p = nullptr, const DeletionPolicy<T> &deletion_policy =
                                        DeletionPolicy<T>())
...
int main() {
  ...
  {
    SmallHeap h;
    // SmartPtr<int, DeletelHeap> p{new(&h) int(42), DeleteHeap<int,
    // SmallHeap>(h)}; // 2 template arguments - does not compile

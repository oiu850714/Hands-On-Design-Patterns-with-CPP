...
protected:
  ...
  MoveCopyRefCounted(const MoveCopyRefCounted &other) : count_(other.count_) {
    if (count_)
      ++(*count_);
  }
  ~MoveCopyRefCounted() {
  ...
  MoveCopyRefCounted(MoveCopyRefCounted &&other) : count_(other.count_) {
...
int main() {
  ...
  {
    SmallHeap h;
    SmartPtr<int, DeleteSmallHeap<int>> p{new (&h) int(42),
                                          DeleteSmallHeap<int>(h)};
    std::cout << *p << std::endl;
  }

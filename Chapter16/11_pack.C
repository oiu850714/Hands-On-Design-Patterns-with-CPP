...
template <typename T, typename V> struct AddSub {
...
template <typename T, typename V> struct MulDiv {
  friend V operator*(V lhs, V rhs) { return V(lhs.get() * rhs.get()); }
  friend V operator*(V lhs, T rhs) { return V(lhs.get() * rhs); }
  friend V operator*(T lhs, V rhs) { return V(lhs * rhs.get()); }
  friend V operator/(V lhs, V rhs) { return V(lhs.get() / rhs.get()); }
...
template <typename T, typename V> struct ExplicitConvertible {
...
template <typename T, typename V> struct ImplicitConvertible {
  // typedef V::value_type value_type; // Does not work: Value is an incomplete
  // type when its base class is instantiated
...
template <typename T, template <typename, typename>
                      class... Policies> // "class" -> "typename" in C++17

...
template <typename T> struct DeleteByFree {
...
public:
  ...
  void deallocate(void *p) { assert(p == mem_); }
...
template <typename T> struct DeleteSmallHeap {
  explicit DeleteSmallHeap(SmallHeap &heap) : heap_(&heap) {}
  DeleteSmallHeap(const DeleteSmallHeap &) = default;
  DeleteSmallHeap &operator=(const DeleteSmallHeap &) = default;
  DeleteSmallHeap(DeleteSmallHeap &&other) : heap_(other.heap_) {
    other.heap_ = nullptr;
  }
  void operator()(T *p) const {
    p->~T();
...
protected:
  ...
  CopyRefCounted(const CopyRefCounted &other) : count_(other.count_) {
    ++(*count_);
  }
  ~CopyRefCounted() {
    --(*count_);
...
struct Debug {
  template <typename P, typename T>
  static void construct(const P *ptr, const T *p) {
    std::cout << "Constructed SmartPtr at " << ptr << ", object "
              << static_cast<const void *>(p) << std::endl;
  }
  template <typename P, typename T>
  static void move(const P *ptr, const P *from, const T *p) {
    std::cout << "Moved SmartPtr at " << ptr << " from " << from << ", object "
              << static_cast<const void *>(p) << std::endl;
  }
  template <typename P, typename T>
  static void copy(const P *ptr, const P *from, const T *p) {
    std::cout << "Copied SmartPtr at " << ptr << " from " << from << ", object "
              << static_cast<const void *>(p) << std::endl;
  }
  template <typename P, typename T>
  static void destroy(const P *ptr, const T *p, bool must_delete) {
    std::cout << "Destroyed SmartPtr at " << ptr << ", object "
              << static_cast<const void *>(p)
              << (must_delete ? " is" : " is not") << " deleted" << std::endl;
  }
};
...
public:
  ...
  SmartPtr(SmartPtr &&other)
      : DeletionPolicy(std::move(other)), CopyMovePolicy(std::move(other)),
        p_(other.p_) {
    DebugPolicy::move(this, &other, p_);
    other.release();
  }
  SmartPtr(const SmartPtr &other)
      : DeletionPolicy(other), CopyMovePolicy(other), p_(other.p_) {
...
class SmartPtrAdapter
    : public SmartPtr<T, DeleteByOperator<T>, CopyRefCounted, DebugPolicy> {
  using SmartPtr<T, DeleteByOperator<T>, CopyRefCounted, DebugPolicy>::
      SmartPtr; // C++11 (in C++03 repeat all constructors)
};

...
public:
  ...
  // Explicit and implicit operators cannot be overloaded even if some are
  // disabled:
  template <typename U = T,
            typename V = std::enable_if_t<ConversionPolicy::implicit_conv, U>>
  operator U *() {
    return p_;
  }
  template <typename U = T,
  ...
  template <typename U = T,
            typename V = std::enable_if_t<ConversionPolicy::explicit_conv, U>>
  explicit operator U *() {
    return p_;
  }
  template <typename U = T,
...
int main() {
  ...
  {
    SmartPtr<int, DeleteByOperator<int>, NoMoveNoCopy, ImplicitRaw> p(
        new int(42));
    std::cout << *p << std::endl;
    int *p1(p);
    int *p2 = p; // Does not compile

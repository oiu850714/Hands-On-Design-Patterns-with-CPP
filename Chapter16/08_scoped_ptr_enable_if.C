...
template <typename T, typename DeletionPolicy = DeleteByOperator<T>,
          typename MovePolicy = MoveForbidden, typename ArrowPolicy = WithArrow>
...
public:
  ...
  template <typename U,
            typename V = std::enable_if_t<
                MovePolicy::can_move && std::is_same<U, SmartPtr>::value, U>>
  SmartPtr(U &&other) : DeletionPolicy(std::move(other)), p_(other.p_) {
    other.release();
  }
  ~SmartPtr() { DeletionPolicy::operator()(p_); }
  void release() { p_ = NULL; }
  // SFINAE works on template functions only
  // std::enable_if_t<ArrowPolicy::have_arrow, T*> operator->() { return p_; }
  // // Does not work
  template <typename U = T>
  std::enable_if_t<ArrowPolicy::have_arrow, U *> operator->() {
    return p_;
  }
  template <typename U = T>

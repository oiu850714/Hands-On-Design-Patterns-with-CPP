...
// Default constructor and destructor, assignment operator, addition and
// subtraction.
struct B {
  int i;
  B &operator=(int j) {
...
auto is_default_constructible = is_valid(
    [](auto &&x) -> decltype(
                     new typename std::remove_reference<decltype(x)>::type) {});
auto is_copyable = is_valid(
    [](auto &&x) -> decltype(
                     new
                     typename std::remove_reference<decltype(x)>::type(x)) {});
...
void check_default_constructible(int i, double x, int *p, const A &a,
                                 const B &b, const C &c, const D &d, const E &e,
                                 const Ptr &ptr, const SmartPtr<int> &iptr) {
  static_assert(decltype(is_default_constructible.f(i))::value,
                "Feature check failed");
  /*
  static_assert(decltype(is_default_constructible(x))::value, "Feature check
  failed"); static_assert(decltype(is_default_constructible(p))::value, "Feature
  check failed"); static_assert(decltype(is_default_constructible(a))::value,
  "Feature check failed");
  static_assert(decltype(is_default_constructible(b))::value, "Feature check
  failed"); static_assert(!decltype(is_default_constructible(c))::value,
  "Feature check failed");
  static_assert(decltype(is_default_constructible(d))::value, "Feature check
  failed"); static_assert(decltype(is_default_constructible(e))::value, "Feature
  check failed"); static_assert(decltype(is_default_constructible(ptr))::value,
  "Feature check failed");
  static_assert(decltype(is_default_constructible(iptr))::value, "Feature check
  failed");
  */
}

/*
void check_copyable( int i, double x, int* p, const A& a, const B& b, const C&
c, const D& d, const E& e, const Ptr& ptr, const SmartPtr<int>& iptr ) {
...
void check_destructible( int i, double x, int* p, const A& a, const B& b, const
C& c, const D& d, const E& e, const Ptr& ptr, const SmartPtr<int>& iptr ) {
    ...
    static_assert(decltype(is_destructible(ptr))::value, "Feature check
failed"); static_assert(decltype(is_destructible(iptr))::value, "Feature check
failed");
}

void check_assignable( int i, double x, int* p, const A& a, const B& b, const C&
c, const D& d, const E& e, const Ptr& ptr, const SmartPtr<int>& iptr ) {
...
void check_assignable0( int i, double x, int* p, const A& a, const B& b, const
C& c, const D& d, const E& e, const Ptr& ptr, const SmartPtr<int>& iptr ) {
    ...
    static_assert(!decltype(is_assignable0(iptr))::value, "Feature check
failed");
}

void check_addable( int i, double x, int* p, const A& a, const B& b, const C& c,
const D& d, const E& e, const Ptr& ptr, const SmartPtr<int>& iptr ) {
...
void check_pointer( int i, double x, int* p, const A& a, const B& b, const C& c,
const D& d, const E& e, const Ptr& ptr, const SmartPtr<int>& iptr ) {

...
void BM_positional_const(benchmark::State &state) {
  for (auto _ : state) {
    REPEAT(
        benchmark::DoNotOptimize(bool(Positional(true, false, true, false)));)
  }
...
void BM_named_const(benchmark::State &state) {
  for (auto _ : state) {
    REPEAT(benchmark::DoNotOptimize(
               bool(Named(Named::Options().SetA(true).SetC(true))));)
  }
  state.SetItemsProcessed(32 * state.iterations());
}
  ...
  for (auto _ : state) {
    REPEAT({
      ...
      benchmark::DoNotOptimize(
          bool(Named(Named::Options().SetA(a).SetB(b).SetC(c).SetD(d))));
      if (i == N)
        i = 0;
    })
  }

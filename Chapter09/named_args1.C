...
void BM_positional_const(benchmark::State &state) {
  for (auto _ : state) {
    REPEAT(benchmark::DoNotOptimize(bool(
        Positional(false, false, false, false, false, false, false, true)));)
  }
...
void BM_positional_var(benchmark::State &state) {
  ...
  for (auto _ : state) {
    REPEAT({
      benchmark::DoNotOptimize(bool(
          Positional(false, false, false, false, false, false, false, v[i++])));
      if (i == N)
        i = 0;
    })
  }

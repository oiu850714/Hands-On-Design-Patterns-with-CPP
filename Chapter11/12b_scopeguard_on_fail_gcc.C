...
private:
  ...
  int uncaught_exceptions() const noexcept {
    return *(reinterpret_cast<int *>(static_cast<char *>(static_cast<void *>(
                                         __cxxabiv1::__cxa_get_globals())) +
                                     sizeof(void *)));
  }
};

template <typename Func, bool on_success, bool on_failure> class ScopeGuard {
public:
...
template <typename Func> auto operator+(ScopeGuardOnExit, Func &&func) {
...
template <typename Func> auto operator+(ScopeGuardOnSuccess, Func &&func) {
...
template <typename Func> auto operator+(ScopeGuardOnFailure, Func &&func) {
...
int main() {
  ...
  if (S.get() != I.get())
    std::cout << "Inconsistent state: " << S.get() << " != " << I.get()
              << std::endl;
  else if (!S.finalized())
